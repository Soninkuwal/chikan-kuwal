...
export function Sidebar({ 
    ...
    if (activeModal === 'chat') {
        return (
            <div className="space-y-4">
                ...
                <Button className="w-full" onClick={() => setChatTarget('admin')}>Chat with Admin</Button>
                <Button className="w-full" variant="secondary" onClick={() => setChatTarget('owner')}>Chat with Owner</Button>
            </div>
        )
    }
     if (activeModal === 'chat_interface') {
        if(chatTarget === 'admin') return <ChatInterface initialMessages={adminChatInitialMessages} storageKey="userAdminChatMessages" currentUser={{name: currentUser.name, avatar: currentUser.avatarFallback}} />;
        if(chatTarget === 'owner') return <ChatInterface initialMessages={ownerChatInitialMessages} storageKey="userOwnerChatMessages" currentUser={{name: currentUser.name, avatar: currentUser.avatarFallback}} />;
    }
  ...
  return (
    <>
              ...
              <nav className="flex flex-col gap-2">
               ...
               <Separator className="my-4" />
                <Button variant="ghost" className="justify-start gap-3 text-base h-12 w-full" onClick={handleChatClick}>
                 ...
                 <Button variant="ghost" className="justify-start gap-3 text-base h-12 w-full" onClick={handleProfileClick}>
      ...
      <WithdrawModal isOpen={activeModal === 'withdraw'} onOpenChange={(open) => !open && setActiveModal(null)} feeType="user" />
